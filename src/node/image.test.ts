...
describe('Image', () => {
  describe('.decode', () => {
    it('is expected to throw error when mime is not supported', async () => {
      ...
      await expect(imageConverter.decode(arrayBuffer)).rejects.toThrow(TypeError);
    });

    // eslint-disable-next-line @typescript-eslint/prefer-readonly-parameter-types
    it.for(Object.entries(testImageData))('is expected to create ImageData from %s', async ([,{ buffer, imageData }]) => {
      const result = await imageConverter.decode(buffer);
  ...
  describe('.encode', () => {
    it('is expected to create PNG from ImageData', async () => {
      const imageArrayBuffer = await imageConverter.encode(testImageData['png.png'].imageData);
    ...
    it.for([
      ...
      const imageArrayBuffer = await imageConverter.encode({
    ...
    it('is expected to create BMP from ImageData', async () => {
      const imageArrayBuffer = await imageConverter.encode(testImageData['bmp.bmp'].imageData, 'image/bmp');
